...
\chapter{Conclusion}\label{chapter:Conclusion}
    ...
    % However, the numerical-relativity simulation under test uses a different numerical code to the simulations in which we train the model.
    % This results in a slight bias between the true $\kappa_2^T$ value and the recovered posteriors and a larger bias in the posteriors for $C$ that can be accounted for in future work by propagating uncertainties in the power-law model.
    ...
    % \par
    % The delay time between the short gamma-ray burst after the inferred merger time for GW17081 is, $\sim 1.7\,\s$.
    % The similarity between this delay time and the observed burst duration for GRB 170817A, $\sim~2\,\s$ informed a model of GRB 170817A where the majority of the delay time was caused by the propagation of the GRB~\cite{Zhang2018}.
    % This model is indifferent to the form of the central engine other than the requirement that the jet is launched and breakout occurs $\lesssim 0.5\,\s$. 
